...
## Estado
Implementada (base operativa + validaciones reforzadas + UI admin).

## Objetivo de la fase
Implementar el ciclo de reservaciones y sus entidades operativas inmediatas:
- reservaciones
- huespedes por reservacion
- logs de check-in/check-out
- cargos por reservacion

## Tablas incluidas en Fase 2
- `reservations`
- `reservation_guests`
...
### Migracion
- `database/migrations/1774000000000_create_hotels_phase2_reservations_tables.ts`

### Modelos
- `app/models/reservation.ts`
- `app/models/reservation_guest.ts`
- `app/models/checkin_checkout_log.ts`
- `app/models/reservation_charge.ts`

### Validadores
- `app/validators/admin/hotels/create_reservation_validator.ts`
- `app/validators/admin/hotels/create_reservation_guest_validator.ts`
- `app/validators/admin/hotels/create_checkin_checkout_log_validator.ts`
- `app/validators/admin/hotels/create_reservation_charge_validator.ts`

### Controladores
- `app/controllers/admin/hotels/reservations_controller.ts`
- `app/controllers/admin/hotels/reservation_guests_controller.ts`
- `app/controllers/admin/hotels/checkin_checkout_logs_controller.ts`
- `app/controllers/admin/hotels/reservation_charges_controller.ts`

### Rutas admin
- `/admin/hotels/reservations` (`index`, `create`, `edit`, `store`, `update`)
- `/admin/hotels/reservation-guests` (`index`, `create`, `edit`, `store`, `update`)
- `/admin/hotels/checkin-checkout-logs` (`index`, `create`, `edit`, `store`, `update`)
- `/admin/hotels/reservation-charges` (`index`, `create`, `edit`, `store`, `update`)

## Validaciones y reglas reforzadas (cierre Fase 2)

### 1) Transiciones de estado de reservacion
Se aplican transiciones validas y se bloquean saltos invalidos.
Ejemplo: `DRAFT -> CHECKED_OUT` no permitido.

### 2) Sincronizacion reserva-habitacion en logs operativos
Los logs operativos actualizan automaticamente estados:
- `CHECK_IN` -> reserva `CHECKED_IN`, habitacion `OCCUPIED`
- `CHECK_OUT` -> reserva `CHECKED_OUT`, habitacion `DIRTY`
- `ROOM_CHANGE_IN` -> habitacion destino `OCCUPIED` y se actualiza `reservation.room_id`
- `ROOM_CHANGE_OUT` -> habitacion origen `DIRTY`
- `NO_SHOW` -> reserva `NO_SHOW` y libera habitacion si corresponde

### 3) Guardas de check-in/check-out
- Check-in solo para reservaciones `CONFIRMED`.
- Check-out solo para reservaciones `CHECKED_IN`.
- Cambios de habitacion solo con reservacion `CHECKED_IN`.

### 4) Cargos: anulacion y estados cerrados
- No se permite crear cargos directamente en `VOIDED`.
- Para pasar a `VOIDED` se requiere `voidReason` y usuario responsable.
- Cargos en `PAID`, `BILLED` o `VOIDED` se consideran cerrados y ya no son editables.

### 5) amountPaid fuera de formulario de reservacion
- `amountPaid` se retira del formulario de reservaciones.
- En creacion de reserva inicia en `0`.
- El saldo (`balanceDue`) se recalcula sin permitir ajuste manual de pago desde reservaciones.
- El flujo de pagos queda reservado para Fase 3.

### 6) Pruebas funcionales agregadas
Archivo: `tests/functional/hotels_phase2_reservations.spec.ts`
Cobertura:
- creacion de reservacion con auditoria
- rechazo de solapamiento por habitacion
- rechazo de segundo huesped `PRIMARY`
- creacion de cargo con calculo de montos
- rechazo de transicion invalida de estado
- sincronizacion de estados con check-in/check-out
- rechazo de check-in cuando no esta `CONFIRMED`
- reglas de anulacion y bloqueo de edicion en cargos cerrados
...
## Referencias
- Fase 1 (tablas base): `docs/requirements/fase-1-diseno-fisico-db.md`
- Requerimiento funcional global: `docs/requirements/hoteles-y-reservas.md`
