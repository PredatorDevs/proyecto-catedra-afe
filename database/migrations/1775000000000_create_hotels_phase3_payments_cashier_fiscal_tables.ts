...
export default class extends BaseSchema {
  ...
  async down() {
    await this.schema.raw('DROP TABLE IF EXISTS \`fiscal_document_payments\`;')
    await this.schema.raw('DROP TABLE IF EXISTS \`fiscal_document_items\`;')
    await this.schema.raw('DROP TABLE IF EXISTS \`fiscal_documents\`;')
    await this.schema.raw('DROP TABLE IF EXISTS \`payment_charge_allocations\`;')
    await this.schema.raw('DROP TABLE IF EXISTS \`payment_reservation_allocations\`;')
    await this.schema.raw('DROP TABLE IF EXISTS \`payment_transactions\`;')
    await this.schema.raw('DROP TABLE IF EXISTS \`payment_proofs\`;')
    await this.schema.raw('DROP TABLE IF EXISTS \`payments\`;')
    await this.schema.raw('DROP TABLE IF EXISTS \`cashier_shifts\`;')
    await this.schema.raw('DROP TABLE IF EXISTS \`payment_methods\`;')
  }
}
